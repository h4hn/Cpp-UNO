...
void intro() //gibt den Intro Screen aus
{
    ...
    else std::cout << "Konnte Intro.txt nicht oeffnen." << std::endl;
}
...
void showRules() // zeigt die Spielregeln
{
    ...
    else std::cout << "Konnte Rules.txt nicht oeffnen." << std::endl;
...
void saveScores(std::vector<Player*>& players) //Speicher die in dieser Runder gemachten Scores in der Datei ab
{
    bool correctinput = false;
    char selection;
    while (!correctinput)
    ...
    if (selection == 'y')
    {
        ...
        else std::cout << "Konnte Scores.txt zum Lesen nicht oeffnen." << std::endl;
        ...
        else std::cout << "Konnte Scores.txt zum Schreiben nicht oeffnen." << std::endl;
    }
}

void showRanking() //Liest das aktuelle Ranking aus der Scores-Datei aus und gibt es aus
{
    clearScreen();
    std::map<int, std::string> ranking;
...
{
    std::ifstream readScore(filePath);
    std::string reading;
    bool isEmpty = true;
    if (readScore.is_open())
    {
